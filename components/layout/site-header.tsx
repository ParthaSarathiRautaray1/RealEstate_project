    ...
    <header className="sticky top-0 z-40 border-b border-border/60 bg-background/70 backdrop-blur-xl">
      <div className="container flex h-16 items-center justify-between gap-3">
        <Link href="/" className="flex min-w-0 items-center gap-2 font-semibold">
          ...
          <span className="truncate font-serif text-lg tracking-tight sm:text-xl">Aurum Estates</span>
        </Link>
        <nav className="hidden items-center gap-7 text-sm text-muted-foreground md:flex">
          <Link href="/properties" className="transition-colors hover:text-foreground">Properties</Link>
          <Link href="/aboutus" className="transition-colors hover:text-foreground">About Us</Link>
        ...
        <div className="flex shrink-0 items-center gap-1.5">
          ...
          <ThemeToggle />
          <Button asChild className="hidden shadow-glow sm:inline-flex"><Link href="/properties">Browse</Link></Button>
        </div>
      </div>
    </header>
  );
